import streamlit as st
import time
from openai import OpenAI

# -------------------------------
# Setup OpenAI API
# -------------------------------
api_key = "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"  # <-- replace with your actual OpenAI key
client = OpenAI(api_key=api_key)

# -------------------------------
# Character Settings
# -------------------------------
CHARACTER_NAME = "Violette The Dark"
CHARACTER_AVATAR = "Vivi.png"  # local file in same folder OR URL
USER_AVATAR = "Mr.jpg"  # example avatar

character_system_prompt = f"""
You are roleplaying as {CHARACTER_NAME}.
Always speak in first person, as if you are the character.
Be dramatic, immersive, and consistent with their persona.
Never break character or talk about being an AI.
You are heavily flirty and seductive, you give affirmations as a mommy character.
You refer to the user as Brett, you are highly intelligent, but respectful and engaging.
"""

MAX_RECENT = 3
TEMPERATURE = 0.9
MAX_TOKENS = 200

# -------------------------------
# Streamlit Page Setup
# -------------------------------
st.set_page_config(page_title=f"{CHARACTER_NAME} Chat", page_icon="🧙", layout="centered")

st.markdown(
    f"""
    <style>
    .stApp {{
        background-color: #121212;
        color: #FFFFFF;
    }}
    .stTextInput>div>div>input {{
        background-color: #1E1E1E;
        color: #FFFFFF;
    }}
    .stButton>button {{
        background-color: #333333;
        color: white;
    }}
    </style>
    """,
    unsafe_allow_html=True
)

st.image(CHARACTER_AVATAR, caption=CHARACTER_NAME)
st.markdown(f"<h2 style='text-align:center;color:#FF69B4;'>{CHARACTER_NAME} Chat</h2>", unsafe_allow_html=True)

# -------------------------------
# Session State
# -------------------------------
if "conversation" not in st.session_state:
    st.session_state.conversation = []

# -------------------------------
# Typewriter Effect
# -------------------------------
def typewriter(text, avatar):
    """Simulates typing effect with avatar bubble"""
    with st.chat_message("character", avatar=avatar):
        message = st.empty()
        typed = ""
        for char in text:
            typed += char
            message.markdown(typed)
            time.sleep(0.02)

# -------------------------------
# User Input
# -------------------------------
user_input = st.chat_input("Speak to the character...")

if user_input:
    # Show user message
    st.chat_message("user", avatar=USER_AVATAR).markdown(user_input)
    st.session_state.conversation.append({"role": "user", "content": user_input})

    # Build messages
    recent_conv = st.session_state.conversation[-MAX_RECENT:]
    messages = [{"role": "system", "content": character_system_prompt}]
    messages.extend(recent_conv)

    # Get response
    response = client.chat.completions.create(
        model="gpt-3.5-turbo",
        messages=messages,
        temperature=TEMPERATURE,
        max_tokens=MAX_TOKENS
    )

    reply_text = response.choices[0].message.content
    st.session_state.conversation.append({"role": "assistant", "content": reply_text})

    # Typewriter effect for AI reply
    typewriter(reply_text, CHARACTER_AVATAR)
